...
namespace cAlgo.Robots
{
    ...
    public class SMACross : Robot
    {
        #region Strategy Parameters

        [Parameter("Fast SMA Period", Group = "Moving Averages", DefaultValue = 9, MinValue = 1)]
        public int FastSmaPeriod { get; set; }

        [Parameter("Slow SMA Period", Group = "Moving Averages", DefaultValue = 21, MinValue = 2)]
        ...
        [Parameter("Instance Label", Group = "Execution", DefaultValue = "SMA_Cross")]
        ...
        protected override void OnStart()
        {
            ...
            {
                Print("WARNING: Fast SMA Period ({0}) should be smaller than Slow SMA Period ({1}).", FastSmaPeriod, SlowSmaPeriod);
            }
            ...
            Print("SMA Cross initialized on {0} [{1}] (Fast: {2}, Slow: {3}).", SymbolName, TimeFrame, FastSmaPeriod, SlowSmaPeriod);
        }
        ...
        protected override void OnBar()
        {
            ...
            int requiredBars = Math.Max(SlowSmaPeriod, AtrLength) + 2;
            if (Bars.Count <= requiredBars)
                return;

            // Index 1 = Most recently closed candle, Index 2 = Candle before index 1
            double fastPrev = _fastSma.Result.Last(2);
            double fastCurr = _fastSma.Result.Last(1);
            double slowPrev = _slowSma.Result.Last(2);
            double slowCurr = _slowSma.Result.Last(1);
